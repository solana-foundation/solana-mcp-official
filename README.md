...
- **Retrieval** ([`lib/services/databricks/`](lib/services/databricks/)): MCP tools query the index via the Databricks managed AI Search MCP server, which retrieves and reranks results server-side. `get_documentation` falls back to a SQL read of the `docs_chunks` Delta table when a source has no published `llms.txt`.
